...
public class InputFuzzy extends JFrame {
    ...
    public static void main(String[] args) {
        EventQueue.invokeLater(new Runnable() {
            public void run() {
                try {
                    InputFuzzy frame = new InputFuzzy();
    ...
    public InputFuzzy() {
        ...
        petriname.setText("1086"); // Default to Controller Port
        petriname.setBounds(10, 205, 285, 20);
        contentPane.add(petriname);

        JTextPane txtName = new JTextPane();
        txtName.setText("P0"); // Default to Ref Input Place
        ...
        btnSend.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent arg0) {
                ...
                try {
                    ...
                    DataToSend.petriObject = new DataFuzzy();
                    if (!txtValue.getText().equals("null")) {
                        // Send as Fuzzy Token (NL, NM, ZR, PM, PL) logic handled in backend or just
                        // value?
                        // Framework InputFuzzy typically sends a Fuzzy object.
                        // We will construct it similarly to GUIs.InputFuzzy
                        DataToSend.petriObject.SetValue(
                                new Fuzzy(new FuzzyVector(0f, 0f, 0f, 0f, 0f), Float.parseFloat(txtValue.getText())));
                    } else {
