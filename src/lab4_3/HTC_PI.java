...
public class HTC_PI {
    public static void main(String[] args) throws FileNotFoundException {
        ...
        pn.PetriNetName = "HTC PI Controller";
        pn.NetworkPort = 1083;
        ...
        FLRS doubleChannelDifferentiator = new FLRS(new FV(FZ.ZR, FZ.ZR), new FV(FZ.NM, FZ.NM), new FV(FZ.NL, FZ.NL),
                new FV(FZ.NL, FZ.NL), new FV(FZ.NL, FZ.NL),
        ...
        p1.SetName("P1");
        pn.PlaceList.add(p1); // Reference WaterTemp (from ORC or set manually)
        ...
        pn.PlaceList.add(p3); // Current WaterTemp
        ...
        // Integral Term Places
        DataFuzzy p9 = new DataFuzzy();
        p9.SetName("P9");
        p9.SetValue(new Fuzzy(0.0F));
        pn.PlaceList.add(p9); // Integral Accumulator
        DataFuzzy p10 = new DataFuzzy();
        p10.SetName("P10");
        pn.PlaceList.add(p10); // Integral Output
        ...
        // T_Integral - Integrate Error (P4 is error?)
        // Wait, T1 uses doubleChannelDifferentiator on P2 and P3.
        // P2 is error (from T0). P3 is current temp.
        // So P4/P5 is derivative of error? Or just error?
        // In HTC.java, T1 uses doubleChannelDifferentiator.
        // Let's assume P4 is the Proportional term (or error).
        // To make it PI, we need to integrate P4.

        PetriTransition t_int = new PetriTransition(pn);
        t_int.TransitionName = "T_Int";
        t_int.InputPlaceName.add("P4");
        t_int.InputPlaceName.add("P9");
        Condition TIntCt1 = new Condition(t_int, "P4", TransitionCondition.NotNull);
        Condition TIntCt2 = new Condition(t_int, "P9", TransitionCondition.NotNull);
        TIntCt1.SetNextCondition(LogicConnector.AND, TIntCt2);
        GuardMapping grdTInt = new GuardMapping();
        grdTInt.condition = TIntCt1;
        ArrayList<PlaceNameWithWeight> inputInt = new ArrayList<>();
        inputInt.add(new PlaceNameWithWeight("P4", 0.1F)); // Integral Gain
        inputInt.add(new PlaceNameWithWeight("P9", 1F)); // Accumulator
        ArrayList<String> OutputInt = new ArrayList<>();
        OutputInt.add("P9");
        OutputInt.add("P10");
        grdTInt.Activations
                .add(new Activation(t_int, doubleChannelAdder, inputInt, TransitionOperation.FLRS, OutputInt));
        t_int.GuardMappingList.add(grdTInt);
        t_int.Delay = 0;
        pn.Transitions.add(t_int);

        // T4 - Sum P (P4) and I (P10) and Previous Output (P6) -> P7, P8
        // Original T4: P4 + P6 -> P7, P8.
        // Now we need P4 + P10 + P6.
        // Let's sum P4 and P10 first into a temp place, say P11.
        // Or just modify T4 to take P10 instead of P4, if P10 was P+I. But P10 is just
        // I.
        // Let's sum P4 (P) and P10 (I) -> P11 (PI).
        DataFuzzy p11 = new DataFuzzy();
        p11.SetName("P11");
        pn.PlaceList.add(p11);

        PetriTransition t_sum = new PetriTransition(pn);
        t_sum.TransitionName = "T_Sum";
        t_sum.InputPlaceName.add("P4");
        t_sum.InputPlaceName.add("P10");
        Condition TSumCt1 = new Condition(t_sum, "P4", TransitionCondition.NotNull);
        Condition TSumCt2 = new Condition(t_sum, "P10", TransitionCondition.NotNull);
        TSumCt1.SetNextCondition(LogicConnector.AND, TSumCt2);
        GuardMapping grdTSum = new GuardMapping();
        grdTSum.condition = TSumCt1;
        ArrayList<PlaceNameWithWeight> inputSum = new ArrayList<>();
        inputSum.add(new PlaceNameWithWeight("P4", 1F));
        inputSum.add(new PlaceNameWithWeight("P10", 1F));
        ArrayList<String> OutputSum = new ArrayList<>();
        OutputSum.add("P11");
        grdTSum.Activations
                .add(new Activation(t_sum, doubleChannelAdder, inputSum, TransitionOperation.FLRS, OutputSum));
        t_sum.GuardMappingList.add(grdTSum);
        t_sum.Delay = 0;
        pn.Transitions.add(t_sum);

        // T4 (Modified) - P11 (PI) + P6 (Prev) -> P7, P8
        ...
        // T3 - Loop back P5 -> P0 (Reset/Next Step)
        ...
        System.out.println("HTC PI Controller started");
        pn.Delay = 500;
