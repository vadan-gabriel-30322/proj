...
public class Activation implements Serializable {
	...
	public Activation(PetriTransition Parent, ArrayList<String> InputPlaceNames, TransitionOperation Condition,
			String Parameter, String OutputPlaceName) {
		util = new Functions();
		this.Parent = Parent;
		this.InputPlaceNames = InputPlaceNames;
		this.OutputPlaceName = OutputPlaceName;
	...
	public void Activate() throws CloneNotSupportedException {
		...
		if (Operation == TransitionOperation.MakeString)
			MakeString();
	...
	private void MakeString() throws CloneNotSupportedException {
		Integer outputIndex = util.GetIndexByName(OutputPlaceName, Parent.Parent.PlaceList);
		PetriObject result = new DataString();
		result.SetName(OutputPlaceName);
	...
	private void PopTaxiToQueue() throws CloneNotSupportedException {
		Integer taxiQueueIndex = util.GetIndexByName(InputPlaceNames.get(0), Parent.Parent.PlaceList);
		Integer userQueueIndex = util.GetIndexByName(InputPlaceNames.get(1), Parent.Parent.PlaceList);
		Integer outputIndex = util.GetIndexByName(OutputPlaceName, Parent.Parent.PlaceList);

		PetriObject taxi = ((CarQueue) ((DataCarQueue) Parent.Parent.PlaceList.get(taxiQueueIndex)).GetValue())
				.PopCartWithoutTarget();
		PetriObject user = ((CarQueue) ((DataCarQueue) Parent.Parent.PlaceList.get(userQueueIndex)).GetValue())
				.PopCartWithoutTarget();

		if (taxi == null || user == null)
			return;

		PetriObject result = (PetriObject) ((DataCar) taxi).clone();
		result.SetName(OutputPlaceName);
