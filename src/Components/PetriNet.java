			...
			try {
				...
				while (!net.stop) {
					try {
						...
						System.out.println("NetworkListener: Connection accepted from " + s.getInetAddress());

						ois = new ObjectInputStream(s.getInputStream());
						System.out.println("NetworkListener: ObjectInputStream created");

						Object obj = ois.readObject();
						System.out.println("NetworkListener: Object received: " + obj.getClass().getName());

						net.inputdata = (DataOverNetwork) obj;
						System.out.println("NetworkListener: Cast to DataOverNetwork successful. Object Name: "
								+ net.inputdata.petriObject.GetName());

						Integer index = net.util.GetIndexByName(net.inputdata.petriObject.GetName(), net.PlaceList);
						System.out.println("NetworkListener: Index in PlaceList: " + index);

						if (index == -1) {
							System.out.println(
									"NetworkListener: ERROR - Place not found: " + net.inputdata.petriObject.GetName());
							continue;
						}
						...
						if (net.inputdata.petriObject.GetType() == PetriObjectType.PetriData) {
							...
							if (net.inputdata.petriObject.GetType() == PetriObjectType.DataNetworkCommand) {
								...
								System.out.println("NetworkListener: Updated PlaceList at index " + index + " with "
										+ net.inputdata.petriObject.toString());
							}
						}
					} catch (ClassNotFoundException e) {
						System.out.println("NetworkListener: ClassNotFoundException");
						e.printStackTrace();
					} catch (Exception e) {
						System.out.println("NetworkListener: Exception: " + e.getMessage());
						e.printStackTrace();
					}
				...
				System.out.println("NetworkListener: IOException outer");
				e.printStackTrace();
			}
		}
	}
